...
public class ViewAllReservations extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        GetEventServicesFunction getEventServicesFunction = new GetEventServicesFunction(ide, null, ViewAllReservations.this, null);
        ...
        for(int j=0;j<GetEventServicesFunction.serviceReservationModel.size();j++) {
            for (int i = ServiceFunctions.serviceModel.size() - 1; i >= 0; i--) {
                if (GetEventServicesFunction.serviceReservationModel.get(j).getService() == ServiceFunctions.serviceModel.get(i).getId()) {
                    ...
                    if(GetEventServicesFunction.serviceReservationModel.get(j).getConfirmed()==0)
                    ...
                    imgView.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            Intent intent = new Intent(ViewAllReservations.this, ViewCorrespondingService.class);
